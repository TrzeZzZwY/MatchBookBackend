...
public class UserBookItemController : ControllerBase
{
    ...
    public async Task<ActionResult<ImageResponse>> AddItemImage(IFormFile file, CancellationToken cancellation)
    {
        ...
        if (result.IsSuccess)
        {
            var a = Url.Content("~/");
            var adress = "https://" +
                Request.HttpContext.Connection.LocalIpAddress.MapToIPv4().ToString() +
                ":" +
                Request.HttpContext.Connection.LocalPort +
                "/api/BookService/Images/" +
                result.Value.ImageId;
            return StatusCode(StatusCodes.Status201Created, new ImageResponse { ImageId = result.Value.ImageId, ImagePath = adress });
        }
